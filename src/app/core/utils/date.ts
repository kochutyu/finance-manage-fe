...
export function calculatingDays(start, end): number {
...
export function startDate(start?: string | number): Date {
  let date = now();
  if (start) {
...
export function endDate(end?: string | number): Date {
  let date = now();
  if (end) {
...
export function getEndDateByAddingMonths(noOfMonths: number, start?: Date | string | number): Date {
...
export function calculatingDaysDiff(start, end): number {
  const oneDay = 24 * 60 * 60 * 1000; // hours*minutes*seconds*milliseconds
  return Math.ceil(Math.abs((start - end) / oneDay));
}

export function isDateSameDay(dateOne, dateTwo): boolean {
  return (
...
export function calculatingHoursDiff(start, end): number {
  return Math.abs(end.getTime() - start.getTime()) / 36e5;
}

function timezoneOffsetPad(num: number): string {
  return (num < 10 ? '0' : '') + num;
}
